    ...
    public class MainViewModel : ViewModelBase
    {
        ...
        public async Task Detect(object arg)
        {
            try
            {
                ...
                string[]? pathes = uiServices.OpenLoadDialog();
                if (pathes != null)
                {
                    foreach (string path in pathes)
                    {
                        var result = await Utils.DetectWithCacheAsync(path, cts.Token);
                    ...
                    //foreach (string path in pathes)
                    //    tasks.Add(Utils.DetectWithCacheAsync(path, cts.Token));
        ...
        public MainViewModel(IUIServices uiServices)
        {
            ErrorVisibility = "Collapsed";
            //imageViews = new();
            imageViews = new ObservableCollection<ImageViewModel>(Utils.GetAllData().Select(elem => new ImageViewModel(elem.Item1, elem.Item2.Count, elem.Item3, elem.Item2)));
            RaisePropertyChanged(nameof(imageViews));
            imageAnalyzer = null;
            ...
            DeleteCommand = new RelayCommand(Delete, _ => !is_detecting);
